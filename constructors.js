...
Movie.prototype.changeRating = function(numRating) {
  let newRating = (this.rating + numRating) / 2;
  this.rating = newRating;
  return this.rating;
};

// var a = new Movie('thing', ' thingy', 123)
// console.log(a.changeRating(1233))
...
User.prototype.removeSavedPost = function(postID) {
  let badPostIndex = this.savedPosts.findIndex(post => post.id === postID)
  this.savedPosts.splice(badPostIndex, 1)
}
...
User.prototype.changePostRating = function(postID, postNewRating) {
  let index = this.savedPosts.findIndex(post => post.id === postID)
  this.savedPosts[index].rating = postNewRating;
}